# -*- coding: utf-8 -*-
"""
Calculate function vector for Sobol indices
TYPE: Function
@author: Pablo Merchan-Rivera
Date of last edition: 15/01/2021
"""
...
modflowMultFile_editable = "pmr_ip_lpf_editable_sobol.dat"
modflowMultFile_processed = "pmr_ip_lpf_sobol.dat"
...
modflowRiverFile_editable = "pmr_bc_riv_editable_sobol.dat"
modflowRiverFile_processed = "pmr_bc_riv_sobol.dat"
...
modflowRechargeFile_processed = "pmr_bc_rch_sobol.dat"
...
def calculate_vector(Mat, numSamples, numSP, numRows, numCols, flag):
    ...
    fMat_AS_60 = np.zeros((numSP, numSamples))
    ...
    for i in range(numSamples):
        ...
        # run MODFLOW model
        success, buff = fp.run_model("MODFLOW_mod_sobol.BAT", "mod_sobol.nam")
        
        if success == True:
            check_success.append(int(1))
        else:
            check_success.append(int(0))
   
        # save all the results of all the simulations
        storeHeads = np.loadtxt("pmr_op_heads_sobol.dat") 
        ...
        fMat_AS_60[:,i] = fed.extract_at(storeHeads, fp_AS_60, numSP, numRows)
    ...
    np.savetxt('logs/log_errors_' + flag + '.txt', np.array(check_success))
    
    return fMat_al, fMat_b1, fMat_b3, fMat_b4, fMat_JS_3, fMat_PS_3, fMat_GS_10, fMat_FS_6, fMat_WW_9, fMat_EW_16, fMat_AW_26, fMat_BW_26, fMat_NW_17, fMat_NW_15, fMat_AS_60, fMat_ep_1, fMat_ep_2, fMat_ep_3, fMat_ep_4 ,fMat_ep_5, fMat_ep_6, fMat_ep_7